...
class Shorty {
     ...
     * @var string
     */
    private $chars = 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789가나다라마바사아자차카타파하';
    ...
    public function set_chars($chars) {
        if (!is_string($chars) || empty($chars)) {
            throw new Exception('잘못된 입력값입니다. (Wrong Input)');
        }
        $this->chars = $chars;
    }
    ...
    public function run() {
        ...
        if (!empty($url)) {
            if (!empty($this->whitelist) && !in_array($_SERVER['REMOTE_ADDR'], $this->whitelist)) {
                $this->error('허가되지 않음. (Not Allowed)');
            }
            ...
            if (preg_match('/^http[s]?\:\/\/[\w]+/', $url)) {
                ...
                $this->error('잘못된 유형입니다 (Bad input).');
            }
        }
        // Lookup by id
        else {
